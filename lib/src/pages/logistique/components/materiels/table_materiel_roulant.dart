...
class TableMaterielRoulant extends StatefulWidget {
  const TableMaterielRoulant(
      {super.key, required this.materielList, required this.controller});
  final List<MaterielModel> materielList;
  final MaterielController controller;

  @override
  State<TableMaterielRoulant> createState() => _TableMaterielRoulantState();
}

class _TableMaterielRoulantState extends State<TableMaterielRoulant> {
    ...
    return PlutoGrid(
      ...
      createHeader: (PlutoGridStateManager header) {
        return Row(
          ...
          children: [
            const TitleWidget(title: "Materiels roulant"),
            Row(
              children: [
                IconButton(
                    onPressed: () {
                      Navigator.pushNamed(
                          context, LogistiqueRoutes.logMaterielRoulant);
                    },
                    icon: Icon(Icons.refresh, color: Colors.green.shade700)),
                PrintWidget(onPressed: () {
                  MaterielXlsx().exportToExcel(widget.materielList);
                  if (!mounted) return;
